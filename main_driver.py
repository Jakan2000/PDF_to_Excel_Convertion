...
def driver(work_book, bank, type, pdf_url, caller):
    ...
    if bank in banks and type in banks[bank]:
        result = banks[bank][type](work_book)
        if result["msg"] is None:
            wb = result["data"]
            response = to_db_and_return_response(wb, pdf_url, caller)
            return response
        else:
            return result
    else:
...
def to_db_and_return_response(wb, pdf_url, caller):
    temp = str(os.path.basename(pdf_url)).replace(".pdf", ".xlsx")
    ...
    try:
        ...
        if caller == "appsmith":
            ...
            file_name = split_file_name[len(split_file_name) - 1].replace(".pdf", ".xlsx")
            file_name = file_name.replace(".PDF", ".xlsx")
            # uploding to minio to get the url
            download_path = os.path.join(download_folder, f"temp_{file_name}")
            temp_new_df.to_excel(download_path)
            bucket_name = "ksv"
            folder_path = "pdf_to_excel_files/"
            temp_excel_url = Excel.minio_upload_pdf(download_path, bucket_name, folder_path)
            os.remove(download_path)
            excel_url = temp_excel_url.split("?")
...
def pdf_to_excel_main(pdf_url, bank, type, caller):
    if bank == "icici" and type == "type4":
        ...
        if result["msg"] is None:
            wb = result["data"]
            responce = to_db_and_return_response(wb, pdf_url, caller)
            return responce
        else:
            return result
    else:
        ...
        if max_column < 2:
            ...
            response = driver(output_workbook, bank, type, pdf_url, caller)

    delete_files_with_criteria(folder_path="C:/Users/Admin/PycharmProjects/pythonProject1/KSV/FormatingExcelFiles", keyword="output", extension='.xlsx')
    delete_files_with_criteria(folder_path="C:/Users/Admin/PycharmProjects/pythonProject1/KSV/FormatingExcelFiles", keyword="temp", extension='.xlsx')
